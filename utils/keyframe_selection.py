...
def keyframe_selection_overlap(gt_depth, w2c, intrinsics, keyframe_list, k, pixels=16000):
        """
...
def keyframe_overlap_test(cur_data, keyframe_list, curr_w2c, pixels=16000):
    key_frame = True
    depth = cur_data['depth']

    width, height = depth.shape[2], depth.shape[1]
    valid_depth_indices = torch.where(depth[0] > 0)
    valid_depth_indices = torch.stack(valid_depth_indices, dim=1)
    indices = torch.randint(valid_depth_indices.shape[0], (pixels,))
    sampled_indices = valid_depth_indices[indices]
    pts = get_pointcloud(depth, cur_data['intrinsics'], curr_w2c, sampled_indices)

    'The overlap of the current frame should be less than 0.85'
    last_keyframe = keyframe_list[-1]
    last_w2c = last_keyframe['est_w2c']
    pts4 = torch.cat([pts, torch.ones_like(pts[:, :1])], dim=1)
    transformed_pts = (last_w2c @ pts4.T).T[:, :3]
    # Project the 3D pointcloud to the keyframe's image space
    points_2d = torch.matmul(cur_data['intrinsics'][:3,:3], transformed_pts.transpose(0, 1))
    points_2d = points_2d.transpose(0, 1)
    points_z = points_2d[:, 2:] + 1e-5
    points_2d = points_2d / points_z
    projected_pts = points_2d[:, :2]
    ...
    # Compute the percentage of points that are inside the image
    percent_inside = mask.sum()/projected_pts.shape[0]
    key_frame &= percent_inside < 0.90
    if not key_frame:
        return key_frame

    'The overlap to the history keyframes should be less than 0.90'
    """
    for keyframeid, keyframe in enumerate(keyframe_list[:-1]):
        # Get the estimated world2cam of the keyframe
        est_w2c = keyframe['est_w2c']
        # Transform the 3D pointcloud to the keyframe's camera space
        pts4 = torch.cat([pts, torch.ones_like(pts[:, :1])], dim=1)
