...
def remove_points(to_remove, params, variables, optimizer):
    ...
    if 'seen_times' in variables.keys():
        variables['seen_times'] = variables['seen_times'][to_keep]

    if 'rgb_colors_importance_weights' in variables.keys() and 'last_rgb_colors' in variables.keys():
        variables['rgb_colors_importance_weights'] = variables['rgb_colors_importance_weights'][to_keep]
        variables['rgb_colors_importance_weights_sum'] = variables['rgb_colors_importance_weights_sum'][to_keep]
        variables['last_rgb_colors'] = variables['last_rgb_colors'][to_keep]

    # depth weights update
    if 'depth_importance_weights' in variables.keys():
        variables['depth_importance_weights'] = variables['depth_importance_weights'][to_keep]
        variables['depth_importance_weights_sum'] = variables['depth_importance_weights_sum'][to_keep]
        variables['last_depth'] = variables['last_depth'][to_keep]

    if 'scale_importance_weights' in variables.keys():
        variables['scale_importance_weights'] = variables['scale_importance_weights'][to_keep]
        variables['scale_importance_weights_sum'] = variables['scale_importance_weights_sum'][to_keep]
        variables['log_scale_last_frame'] = variables['log_scale_last_frame'][to_keep]

    if 'timestep' in variables.keys():
        variables['timestep'] = variables['timestep'][to_keep]
    return params, variables

def remove_points_wo_optimizer(to_remove, params, variables):
    to_keep = ~to_remove
    keys = [k for k in params.keys() if k not in ['cam_unnorm_rots', 'cam_trans']]
    for k in keys:
        new_params = torch.nn.Parameter(params[k][to_keep].requires_grad_(True))
        params[k] = new_params
    variables['means2D_gradient_accum'] = variables['means2D_gradient_accum'][to_keep]
    variables['denom'] = variables['denom'][to_keep]
    variables['max_2D_radius'] = variables['max_2D_radius'][to_keep]
...
def prune_gaussians(params, variables, optimizer, iter, prune_dict):
    ...
    if iter <= prune_dict['stop_after']:
        if (iter >= prune_dict['start_after']) and (iter % prune_dict['prune_every'] == 0):
            ...
            removed_num = to_remove.sum().item()
    ...
    return params, variables, removed_num
