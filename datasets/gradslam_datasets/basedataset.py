...
class GradSLAMDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.crop_edge is not None and self.crop_edge > 0:
            color = color[self.crop_edge:-self.crop_edge, self.crop_edge:-self.crop_edge, :]
            depth = depth[self.crop_edge:-self.crop_edge, self.crop_edge:-self.crop_edge]

        K = datautils.scale_intrinsics(K, self.height_downsample_ratio, self.width_downsample_ratio)
        intrinsics = torch.eye(4).to(K)
        intrinsics[:3, :3] = K
        # divide the cx and cy
        intrinsics[0, 2] -= self.crop_edge
        intrinsics[1, 2] -= self.crop_edge
