...
for idx in range(start, end, 1):
    ...
    # render the image
    frame_pts = transform_to_frame_offline(params, idx, False, False)
    rendervar, _ = transformed_params2rendervar(params, frame_pts)
    im, radius, _, _, = Renderer(raster_settings=cam)(**rendervar)

    # visualize the rendered image `im`` and `color` together
    fig, axes = plt.subplots(1, 2, figsize=(12, 6))
    axes[0].imshow(im.permute(1, 2, 0).detach().cpu().numpy())
    axes[0].set_title("Rendered Image")
    axes[1].imshow(color.permute(1, 2, 0).detach().cpu().numpy())
    axes[1].set_title("Color Image")
    # add indx in the figure
    fig.suptitle("Frame Index: {}".format(idx))
    # save the figure
    plt.savefig("/home/shuo/projects/shuo/implicit_reconstruction/SplaTAM/re_render_resuls/frame_{}.png".format(idx))
    plt.close()
